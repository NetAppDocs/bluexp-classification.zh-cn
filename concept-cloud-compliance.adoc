...
* 查看和修改 https://["Azure 信息保护（ AIP ）标签"^] 在文件中。
...
* 通过确定可以分层到成本较低的对象存储的非活动数据、节省存储成本。 https://["了解有关从Cloud Volumes ONTAP 系统分层的更多信息"^]。 https://["了解有关从内部ONTAP 系统分层的更多信息"^]。
...
* 使用BlueXP分类的成本取决于要扫描的数据量。BlueXP分类在BlueXP工作空间中扫描的前1 TB数据可免费使用30天。这包括所有工作环境和数据源中的所有数据。要在这之后继续扫描数据，需要订阅 AWS ， Azure 或 GCP Marketplace 或 NetApp 提供的 BYOL 许可证。请参见 https://["定价"^] 了解详细信息。
+
...
* BlueXP分类要求您已部署BlueXP Connector。在许多情况下、由于您在BlueXP中使用的其他存储和服务、您已经有了一个Connector。Connector 实例会从部署该实例的云提供商处收取费用。请参见 https://["为每个云提供商部署的实例类型"^]。如果在内部部署系统上安装 Connector ，则不需要任何成本。
...
* https://["AWS ： Amazon EC2 定价"^]
* https://["Microsoft Azure ：带宽定价详细信息"^]
* https://["Google Cloud ：存储传输服务定价"^]
...
在云中部署BlueXP分类时、BlueXP会将实例部署在与连接器相同的子网中。 https://["了解有关连接器的更多信息。"^]

image:diagram_cloud_compliance_instance.png["显示在云提供商中运行的BlueXP实例和BlueXP分类实例的示意图。"]

请注意以下有关默认实例的信息：

* 在AWS中、BlueXP分类在上运行 https://["m6i.4xlarge实例"^] 使用500 GiB GP2磁盘。操作系统映像为 Amazon Linux 2 。在AWS中部署时、如果您要扫描少量数据、则可以选择较小的实例大小。
...
https://["了解有关BlueXP角色的更多信息"^] 以及操作方法 https://["添加具有特定角色的用户"^]。
