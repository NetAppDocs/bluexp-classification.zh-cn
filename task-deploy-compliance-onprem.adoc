...
summary: 完成以下几个步骤、在网络中的Linux主机或云中可访问Internet的Linux主机上部署Cloud Data sense。 
---
= 在可访问Internet的主机上部署Cloud Data sense
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
完成以下几个步骤、在网络中的Linux主机或云中可访问Internet的Linux主机上部署Cloud Data sense。
...
. 根据您是在云中的实例还是内部实例上部署、单击相应的*部署*按钮以启动Data sense部署向导。
+
image:screenshot_cloud_compliance_deploy_onprem.png["选择按钮在云中或内部环境中的计算机上部署Cloud Data sense的屏幕截图。"]

. 此时将显示_Deploy Data sense on premises_对话框。复制提供的命令并将其粘贴到文本文件中、以便稍后使用、然后单击*关闭*。例如：
+
