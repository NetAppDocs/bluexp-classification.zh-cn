...
[role="quick-margin-para"]
确保您的环境可以满足前提条件。其中包括实例的出站 Internet 访问，通过端口 443 在 Connector 和 Cloud Data sense 之间建立连接等。  Cloud Manager and Data Sense prerequisites,请参见完整列表。
...
|===
--
确保 Cloud Manager 具有所需权限:: 确保 Cloud Manager 有权为 Cloud Data sense 实例部署资源并创建安全组。您可以在中找到最新的 Cloud Manager 权限 https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions.html["NetApp 提供的策略"^]。
...
请参见  the Linux host system,准备 Linux 主机系统 和  Cloud Manager and Data Sense prerequisites,查看前提条件 了解部署 Cloud Data sense 之前的完整要求列表。
...
* （可选）验证系统是否已安装两个必备软件包（ Docker 引擎和 Python 3 ）。如果此软件尚未安装在系统上，安装程序将安装此软件。
* 确保您在 Linux 系统上具有 root 权限。
* 验证您的环境是否满足要求  Cloud Manager and Data Sense prerequisites,权限和连接。
