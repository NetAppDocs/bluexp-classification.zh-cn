...
+
image:screenshot_s3_enable_compliance.png["从 \" 服务 \" 面板启用 Cloud Data sense 服务的屏幕截图"]

. 出现提示时，将 IAM 角色分配给具有的 Cloud Data sense 实例 <<查看 S3 前提条件,所需权限>>。
+
image:screenshot_s3_compliance_iam_role.png["为 Cloud Data sense 输入 AWS IAM 角色的屏幕截图"]
