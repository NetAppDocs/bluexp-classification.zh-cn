...
如果您还没有 Connector ，请立即创建一个 Connector 。请参见 https://["在 AWS 中创建连接器"^]， https://["在 Azure 中创建连接器"^]或 https://["在 GCP 中创建连接器"^]。

[role="quick-margin-para"]
您也可以 https://["在内部安装 Connector"^] 在网络中的Linux主机或云中的Linux主机上。
...
如果您还没有 Connector ，请在云提供商中创建一个 Connector 。请参见 https://["在 AWS 中创建连接器"^] 或 https://["在 Azure 中创建连接器"^]或 https://["在 GCP 中创建连接器"^]。在大多数情况下、您可能会在尝试激活BlueXP分类之前设置连接器、因为MOST https://["BlueXP功能需要使用Connector"]但在某些情况下，您需要立即设置一个。
...
请注意，您也可以 https://["在内部安装 Connector"^] 在网络或云中的 Linux 主机上。一些计划在本机安装BlueXP分类的用户也可以选择在本机安装Connector。
...
https://["请参见有关在政府区域部署Connector的详细信息"^]。
...
--
====
确保BlueXP具有所需权限:: 确保BlueXP有权为BlueXP分类实例部署资源和创建安全组。您可以在中找到最新的BlueXP权限 https://["NetApp 提供的策略"^]。
...
Azure和Azure政府部署还需要其他入站和出站安全组规则。请参见 https://["Azure 中连接器的规则"^] 了解详细信息。
...
* https://["AWS 文档： Amazon EC2 服务配额"^]
* https://["Azure 文档：虚拟机 vCPU 配额"^]
* https://["Google Cloud 文档：资源配额"^]
