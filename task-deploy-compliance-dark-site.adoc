...
* 从BlueXP自动升级软件
+
BlueXP Connector和BlueXP分类都需要定期手动升级才能启用新功能。您可以在BlueXP分类UI页面底部看到BlueXP分类版本。检查 link:whats-new.html["BlueXP分类发行说明"] 以查看每个版本中的新功能以及是否需要这些功能。然后，您可以按照以下步骤进行操作 https://["升级BlueXP Connector"^] 和 <<升级BlueXP分类软件,升级BlueXP分类软件>>。
...
如果您尚未在脱机内部站点上安装 Connector ， https://["部署连接器"^] 现在在 Linux 主机上。
...
如果您尚未在脱机内部站点上安装BlueXP Connector、 https://["部署连接器"^] 在脱机站点的 Linux 主机上。
...
* 确保Connector有权为BlueXP分类实例部署资源和创建安全组。您可以在中找到最新的BlueXP权限 https://["NetApp 提供的策略"^]。
...
.步骤
. 在已配置Internet的系统上、从下载BlueXP分类软件 https://["NetApp 支持站点"^]。您应选择的文件名为 * Datasis-offline-bundle-<version>.tar.gz* 。
...
* 验证BlueXP Connector软件是否已升级到最新可用版本。 https://["请参见 Connector 升级步骤"^]。
