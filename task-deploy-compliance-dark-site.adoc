...
如果您需要非常安全的Cloud Manager安装、但也希望从OneDrive帐户或SharePoint Online帐户扫描本地数据、则可以使用Data sense脱机安装程序并提供对一些选定端点的Internet访问。请参见 <<SharePoint and OneDrive special requirements,SharePoint 和 OneDrive 的特殊要求>> 了解详细信息。
...
.开始之前
* 数据感知软件一次可升级一个主要版本。例如、如果您安装的是1.14x版、则只能升级到1.15-x如果您有几个主要版本，则需要多次升级此软件。
