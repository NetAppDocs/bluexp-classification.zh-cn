...
* <proximity_words> =在所发现模式之前或之后所搜索模式的300个字符以内的字词、以帮助细化结果的准确性。每个词用逗号(、)分隔。
...
如果要将regex强制添加到Data sense中、您可以使用命令行末尾的*—f*选项、即使我们认为该regex过于宽泛也是如此。或者、您也可以使用"-p"选项尝试细化搜索结果。



=== 示例

产品ID是一个12位数、以201开头；例如"201123456789"。因此、正则表达式为*。b201\d｛9｝\b*。您希望Data sense UI中的文本将此模式标识为*内部产品ID*。我们将添加接近词"product id"和"identifier"。
